...
class LocationAnalyzer:
    ...
    def _fetch_admin_hierarchy_batch(self, locations: List[Dict]) -> List[Dict]:
        ...
        # Build a single query for all locations
        query_parts = []
        for i, location in enumerate(locations):
            lat = location.get('latitude')
            lon = location.get('longitude')
        ...
        try:
            ...
            print(f"Warning: Failed to fetch hierarchy batch: {str(e)}")
            return locations
        except Exception as e:
            print(f"Warning: Unexpected error fetching hierarchy batch: {str(e)}")
            return locations
    ...
    def run(self):
        ...
        try:
            ...
            all_locations = self.estimate_populations(all_locations, fetch_hierarchy=False)
